...
def generar_pdf_oficial(datos: dict[str, Any]) -> bytes:
    """
    ...
    pdf.cell(0, 5, "Certificado técnico de cumplimiento medioambiental", align="C", ln=1)
    pdf.ln(4)
    ...
    pdf.multi_cell(
        0,
        6,
        "D. [REPRESENTANTE LEGAL], en calidad de administrador, certifica que los datos "
        "consignados corresponden a los registros digitales auditados por el sistema, "
        "con arreglo a los estándares de digitalización exigidos para ayudas y subvenciones.",
        align="J",
    )
    pdf.ln(6)
    ...
    pdf.set_text_color(*EMERALD_600)
    pdf.cell(0, 6, "Indicadores auditados", ln=1)
    pdf.set_draw_color(*ZINC_200)
    pdf.set_fill_color(250, 250, 250)
    pdf.set_font(body, "B", 8)
    pdf.set_text_color(*ZINC_800)
    pdf.cell(100, 8, "Indicador", 1, 0, "L", True)
    pdf.cell(45, 8, "Valor", 1, 0, "C", True)
    pdf.cell(45, 8, "Unidad", 1, 1, "C", True)

    pdf.set_font(body, "", 9)
    pdf.set_fill_color(255, 255, 255)
    pdf.cell(100, 9, "Digitalización documental", 1, 0, "L")
    pdf.cell(45, 9, str(datos.get("n_tickets", "")), 1, 0, "C")
    pdf.cell(45, 9, "Uds", 1, 1, "C")

    pdf.cell(100, 9, "Ahorro materia prima (papel)", 1, 0, "L")
    pdf.cell(45, 9, f"{float(datos.get('papel_kg', 0)):.2f}", 1, 0, "C")
    pdf.cell(45, 9, "kg", 1, 1, "C")

    pdf.set_text_color(*EMERALD_600)
    pdf.set_font(body, "B", 9)
    pdf.cell(100, 9, "Reducción huella carbono (CO2)", 1, 0, "L")
    ...
    pdf.set_font(body, "", 9)
    pdf.set_text_color(*ZINC_800)
    pdf.cell(0, 8, "Firma y sello digital (representación interna)", ln=1)

    pdf.set_y(-22)
    draw_eco_footer_icon(pdf, pdf.w / 2 - 3, pdf.h - 17)
    pdf.set_font(body, "", 7)
    pdf.set_text_color(*EMERALD_600)
    pdf.cell(0, 4, "Compromiso ecológico · AB Logistics OS", align="C", ln=1)
    pdf.set_text_color(*ZINC_500)
    pdf.set_font(body, "", 7)
    pdf.cell(
        0,
        4,
        f"Doc. ID {datetime.datetime.now().strftime('%Y%m%d')}-ESG · Uso administrativo",
        align="C",
        ln=1,
    )
