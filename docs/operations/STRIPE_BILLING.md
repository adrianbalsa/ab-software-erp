...
| `STRIPE_PRICE_STARTER` / `STRIPE_PRICE_PRO` / `STRIPE_PRICE_ENTERPRISE` | IDs de precio (`price_…`) alineados a los productos SaaS (nombres comerciales **Compliance** / **Operational** / **Institutional**; catálogo orientativo **149 € / 449 € / desde 1.200 €+** / mes + IVA). |
...
1. **Productos y precios**  
   Crear precios recurrentes para **Compliance**, **Operational** e **Institutional** (slugs técnicos `starter` / `pro` / `enterprise`) y, si aplica, líneas de add-on (OCR Pack, Webhooks B2B Premium, LogisAdvisor IA Pro). Copiar los `price_*` a las variables `STRIPE_PRICE_*`.
