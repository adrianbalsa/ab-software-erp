...
contenido_correcto = '''import streamlit as st
import pandas as pd
...
def generar_pdf_completo(datos):
    ...
    pdf.cell(100, 4, 'AB Software Solutions S.L.', 0, 0)
    pdf.set_xy(110, pdf.get_y())
    pdf.cell(0, 4, datos['cliente'], 0, 1)

    pdf.cell(100, 4, 'CIF: B-12345678', 0, 0)
    pdf.set_xy(110, pdf.get_y())
...
def render_presupuestos_view(db):
    ...
    if 'empresa_id' not in st.session_state:
        st.error("⚠️ Error Crítico: No se ha detectado el ID de empresa en la sesión.")
        return

    eid = st.session_state.empresa_id
...
L'équipe commerciale."""
    ...
    with tab_gestion:
        ...
        if df_hist.empty:
            ...
            st.dataframe(
                df_hist[["fecha", "cliente", "titulo", "total_final", "moneda", "estado", "num_factura"]],
                use_container_width=True
            )
            ...
            with col_accion_fac:
                ...
                if not pendientes.empty:
                    ...
                    if st.button("✅ GENERAR FACTURA OFICIAL"):
                        ...
                        datos_hash = {
                            'nif_empresa': st.session_state.get('nif_empresa',
                                                               st.secrets.get('NIF_EMPRESA', 'B00000000')),
            ...
            with col_accion_pdf:
                ...
                if opcion_pdf:
                    ...
                    datos_reporte = {
                        ...
                        "fecha": str(row.get("fecha_factura") or row.get("fecha", datetime.date.today())),
                        "total": row["total_final"],
                        "moneda": row["moneda"],
                        "items": items_dec,
                        "num_factura": row.get("num_factura"),
                        "hash_factura": row.get("hash_factura"),
                        "nif_empresa": row.get("nif_empresa"),
