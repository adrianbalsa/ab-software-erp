...
export function Finanzas() {
  ...
  const margen =
    data?.margen_km_eur != null ? formatEUR(data.margen_km_eur) : "— (sin km facturados)";
  ...
  return (
    <AppShell active="finanzas">
      <header className="h-16 ab-header border-b border-slate-200/80 flex items-center justify-between px-8 z-10 shrink-0">
        <div>
          <h1 className="text-2xl font-bold text-slate-800 tracking-tight">Dashboard financiero</h1>
      ...
      <main className="p-8 space-y-8 flex-1 overflow-y-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-6">
          <div className="ab-kpi ab-kpi-accent">
            ...
            <p className="ab-kpi-value text-[#0b1224]">{loading ? "…" : data ? formatEUR(data.ingresos) : "—"}</p>
            <p className="text-xs text-slate-500 mt-2">Bases imponibles facturadas (sin IVA)</p>
          </div>
          <div className="ab-kpi">
            <p className="ab-kpi-label">Gastos operativos</p>
            <p className="ab-kpi-value">{loading ? "…" : data ? formatEUR(data.gastos) : "—"}</p>
          ...
          <div className="ab-kpi ab-kpi-accent">
            ...
            <p className="ab-kpi-value text-[#2563eb]">{loading ? "…" : data ? formatEUR(data.ebitda) : "—"}</p>
            <p className="text-xs text-slate-500 mt-2 flex items-center gap-1">
        ...
        <div className="ab-card rounded-2xl p-6">
          <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-2 mb-6">
            <div>
              <h2 className="text-lg font-bold text-[#0b1224]">Ingresos vs gastos (últimos 6 meses)</h2>
              <p className="text-sm text-slate-500">
                Ingresos por <code className="text-xs">fecha_emision</code> de facturas; gastos por{" "}
                <code className="text-xs">fecha</code> de tickets
              </p>
            </div>
          </div>
            ...
            {chartData.length === 0 && !loading ? (
              ...
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={chartData} margin={{ top: 8, right: 8, left: 0, bottom: 0 }}>
                  ...
                  <Tooltip formatter={(value) => formatEUR(Number(value ?? 0))} contentStyle={{ borderRadius: 12, borderColor: "#e2e8f0" }} />
                  <Legend />
                  <Bar dataKey="ingresos" name="Ingresos" fill="#2563eb" radius={[4, 4, 0, 0]} />
                  <Bar dataKey="gastos" name="Gastos" fill="#0b1224" radius={[4, 4, 0, 0]} />
                </BarChart>
              </ResponsiveContainer>
            )}
          </div>
        ...
        <div className="ab-card rounded-2xl p-6">
          <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-2 mb-6">
            <div>
              <h2 className="text-lg font-bold text-[#0b1224]">Cash-flow en tiempo real</h2>
              <p className="text-sm text-slate-500">Actualización automática cada 20 segundos.</p>
            </div>
          </div>
          <div className="h-80 w-full min-w-0">
            {cashFlowData.length === 0 && !loading ? (
              <p className="text-sm text-slate-500">Sin datos de tesorería para graficar.</p>
            ) : (
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={cashFlowData} margin={{ top: 8, right: 8, left: 0, bottom: 0 }}>
                  ...
                  <Tooltip formatter={(value) => formatEUR(Number(value ?? 0))} contentStyle={{ borderRadius: 12, borderColor: "#e2e8f0" }} />
                  <Legend />
                  <Line type="monotone" dataKey="facturado" name="Facturado" stroke="#2563eb" strokeWidth={2} dot={false} />
                  <Line type="monotone" dataKey="cobrado" name="Cobrado" stroke="#059669" strokeWidth={2} dot={false} />
                  <Line type="monotone" dataKey="neto" name="Neto" stroke="#0b1224" strokeWidth={2} dot={false} />
                </LineChart>
              </ResponsiveContainer>
            )}
          </div>
