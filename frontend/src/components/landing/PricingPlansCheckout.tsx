...
export function PricingPlansCheckout({ variant }: { variant: Variant }) {
  ...
  const handlePrimaryCta = async (tier: PublicPlanTier) => {
    ...
    try {
      const response = await apiFetch(`${API_BASE}/api/v1/stripe/crear-sesion-checkout`, {
        method: "POST",
    ...
    >
      <div className={variant === "marketing-section" ? "text-center mb-12" : "text-center mb-10 sm:mb-12"}>
        <TitleTag
          className={
            variant === "full-page"
              ? "text-3xl font-bold tracking-tight text-zinc-900 dark:text-zinc-100 sm:text-4xl"
              : "text-2xl font-bold tracking-tight text-zinc-900 dark:text-zinc-100 sm:text-3xl"
          }
        >
          {variant === "full-page" ? lp.title : l.title}
        </TitleTag>
        <p className="mt-3 text-pretty text-sm text-zinc-600 dark:text-zinc-300 sm:text-base max-w-3xl mx-auto">
          {variant === "full-page" ? lp.subtitle : l.subtitle}
        </p>
        {variant === "full-page" && !empresaId ? (
          <p className="mt-4 text-pretty text-sm text-amber-700 dark:text-amber-200/90 max-w-2xl mx-auto">{lp.empresaRequiredHint}</p>
        ) : null}
          ...
          return (
            <div
              ...
              className={`relative flex flex-col rounded-3xl border p-8 transition-shadow duration-300 ${
                tier.highlight
                  ? "border-emerald-400/60 bg-gradient-to-b from-emerald-50 to-white shadow-2xl shadow-emerald-500/10 ring-2 ring-emerald-500/30 dark:from-emerald-500/10 dark:to-zinc-900/90 md:-translate-y-1 md:scale-[1.02]"
            ...
            >
              ...
              <h3 className="text-lg font-bold tracking-tight text-zinc-900 dark:text-zinc-100">{name}</h3>
              <p className="mt-2 text-sm leading-relaxed text-zinc-600 dark:text-zinc-400">{tierTagline(catalog, tier.slug)}</p>
              <div className="mt-6 flex items-baseline gap-1">
                <span className="text-4xl font-extrabold text-zinc-900 dark:text-zinc-100">{tierPriceLabel(tier)}</span>
              ...
              <ul className="mt-6 flex-1 divide-y divide-zinc-200/90 dark:divide-zinc-800/80 rounded-2xl border border-zinc-200/90 dark:border-zinc-800/80 bg-surface-elevated text-sm">
                {l.features.map((feature, index) => (
                  <li key={feature} className="flex items-center justify-between gap-2 px-3 py-2.5 text-zinc-600 dark:text-zinc-300">
                    <span>{feature}</span>
                    {tier.includes[index] ? (
