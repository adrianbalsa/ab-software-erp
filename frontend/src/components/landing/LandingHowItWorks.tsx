        ...
        <div className="relative">
          ...
          <div className="space-y-10 md:space-y-0 md:grid md:grid-cols-3 md:gap-8">
            {steps.map((step, i) => (
              ...
              >
                <div className="relative z-10 flex h-14 w-14 items-center justify-center rounded-2xl border border-zinc-300 dark:border-zinc-700 bg-white text-blue-500 shadow-lg shadow-blue-500/10 dark:bg-zinc-900 dark:text-blue-400">
                  <step.icon className="h-7 w-7" />
                </div>
                <span className="mt-4 text-xs font-semibold uppercase tracking-widest text-emerald-500/90">
                  {l.stepLabel} {i + 1} · {step.time}
                </span>
                <h3 className="mt-2 text-lg font-bold tracking-tight text-zinc-900 dark:text-zinc-100">{step.title}</h3>
