 ...
 * (`plan_features`, precios EUR de referencia, `max_vehiculos`, `max_workspace_seats`).
 *
 * Segmentación comercial 2026: Compliance (starter) / Operational (pro) / Institutional (enterprise).
 *
 * Fuente canónica para el build de Next en Vercel cuando Root Directory es `frontend/`
 * (no hay acceso a `../shared` en el sandbox de build).
 *
 * Matriz de 6 filas (orden = `landing.pricing.features` en i18n):
 * 1. VeriFactu — verifactu (todos true)
 * 2. Motor financiero / EBITDA avanzado — math_engine (Compliance false; Operational+ true)
 * 3. Portal del chófer — capacidad operativa móvil (todos true)
 * 4. Cotizador inteligente — ligado a motor financiero (Compliance false; Operational+ true)
 * 5. Control de vencimientos — Operational+ (Compliance false)
 * 6. Liquidaciones automáticas — solo Institutional (enterprise)
 */
